class AdminActionsController < ApplicationController
    ...
    def delivery_csv
        ...
        @deliveries.each do |c|
            @data.push({customer_id:c.id,email:c.email,name:c.name,address:c.delivery_address,phone_number:c.phone_number, reg_mon:"#{[nil,'',0].include?(c.regular_meals_on_monday) ? '' : c.regular_meals_on_monday.to_s+' Reg'}" , grn_mon:"#{[nil,'',0].include?(c.green_meals_on_monday) ? '' : c.green_meals_on_monday+ ' Grn'}", reg_thu:"#{[nil,'',0].include?(c.regular_meals_on_thursday) ? '' : c.regular_meals_on_thursday+' Reg'}", grn_thu:"#{[nil,'',0].include?(c.green_meals_on_thursday) ? '' : c.green_meals_on_thursday+' Grn'}",no_pork:"#{c.no_pork ? 'No Pork' : ''}",no_beef:"#{c.no_beef ? 'No Beef' : ''}",no_poultry:"#{c.no_poultry ? 'No poultry' : ''}",special_delivery_instructions:c.special_delivery_instructions,selected_hub:c.hub,monday_delivery_hub:c.monday_delivery_hub,thursday_delivery_hub:c.thursday_delivery_hub,delivery_time:c.delivery_time,delivery_boundary:c.delivery_boundary})
        end
