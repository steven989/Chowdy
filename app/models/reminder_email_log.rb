class ReminderEmailLog < ActiveRecord::Base
    ...
    def self.generate_customer_list(duration)
        ...
        Customer.where(active?:["No","no",nil,""])
        Customer.find_by_sql(
            ...
            Right Join 
                    ...
                    Where (a.active? in ('No',no,'') or a.active? is NULL) and (c.stripe_customer_id is NULL)
                    Group by a.id
                    ) b on a.id = b.customer_id
            Where 
                (b.latest_reminder_email_sent is NULL or ((b.latest_reminder_email_sent < b.latest_cancel_request_created_at) AND (b.latest_reminder_email_sent < ( current_date + (-65 * interval '1 day'))) ))
                AND b.latest_cancel_request_created_at < ( current_date + (-65 * interval '1 day'))
