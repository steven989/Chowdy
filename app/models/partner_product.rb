class PartnerProduct < ActiveRecord::Base
    ...
    def self.products_to_display
        PartnerProduct.find_by_sql("Select a.* From partner_products a left join partner_product_order_summaries b on a.id = b.product_id and b.delivery_date = '#{PartnerProductDeliveryDate.first.delivery_date}' where coalesce(b.ordered_quantity,0) < a.max_quantity and a.available = true Order By a.position asc, a.created_at desc")
    end
